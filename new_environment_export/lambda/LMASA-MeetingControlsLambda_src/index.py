...
USER_POOL_ID = environ.get("COGNITO_USER_POOL_ID")
...
def resolve_usernames(emails: list) -> list:
    """
    Given a list of email addresses, return the corresponding Cognito usernames.
    Falls back to the original email if no Cognito user is found.
    """
    if not USER_POOL_ID:
        logger.warning("COGNITO_USER_POOL_ID not set, storing emails as-is")
        return emails
    usernames = []
    for email in emails:
        try:
            ...
            if users:
                usernames.append(users[0]["Username"])
            else:
                logger.warning("No Cognito user found for email %s, storing email as fallback", email)
                usernames.append(email)
        except Exception as e:
            logger.warning("Could not resolve username for %s: %s", email, e)
            usernames.append(email)
    return usernames
...
def share_meeting(appsync_session, schema, callid, listPK, listSK, recipients, owner):
    ...
    if new_recipients:
        new_recipients = resolve_usernames(new_recipients)
